...
import numpy as np
import cv2
from uart_hr_receiver import find_footer
...
if __name__ == "__main__":
    ...
    for i in range(args.n_frames):
        ...
        time_stamp = np.frombuffer(data_buffer, dtype=np.uint8, count=args.timestamp_size,
                                   offset=offset)
        time_stamp = int.from_bytes(data_buffer[offset], "little", signed="False")
        ...
        if args.output_dir is not None:
            ...
            out_file_path = os.path.join(args.output_dir, '%08d_%08d.png' % (i, timestamp))
